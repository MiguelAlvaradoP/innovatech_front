...
import { User, Shield, Mail, Trash2, Edit3, Fingerprint } from 'lucide-react';
...
const UsuariosList = ({ onEditar }) => {
  ...
  const cargarUsuarios = () => {
    // Ajusta la URL según tu microservicio de usuarios (ej: puerto 8081)
  ...
  const eliminarUsuario = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:8085/api/usuarios/${id}`, { method: 'DELETE' });
      if (res.ok) setUsuarios(usuarios.filter(u => u.id !== id));
    } catch (err) { alert("Error de red"); }
  };

  if (loading) return <div className="text-center py-10 text-blue-500 font-black animate-pulse">ACCEDIENDO AL DIRECTORIO...</div>;
      ...
      {usuarios.map((u) => (
        <div key={u.id} className="bg-slate-900 border border-slate-800 rounded-[2rem] p-6 hover:border-blue-500/40 transition-all group relative">
          ...
          <div className="flex flex-col items-center text-center">
            ...
            <span className="text-[10px] px-3 py-1 bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-full font-black mt-2 tracking-widest uppercase">
              {u.rol || 'USER'}
            </span>
          </div>
          ...
          <div className="mt-6 pt-6 border-t border-slate-800 space-y-3">
            ...
            <div className="flex items-center gap-3 text-slate-500">
              <Fingerprint size={14} />
              <span className="text-[10px] font-mono uppercase">UUID: {u.id}</span>
            </div>
          </div>
        </div>
      ))}
